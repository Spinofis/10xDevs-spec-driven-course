...
public sealed class GetTripPlanTests
{
    ...
    public async Task GetTripPlan_Returns200_WithPlanAndCorrelationId()
    {
        ...
        Assert.StartsWith("09:00", items[0].GetProperty("startTime").GetString());
    ...
    private async Task SeedPlanAsync(Guid tripId)
    {
        ...
        var firstItem = PlanItem.Create(
            tripId,
            itemDate: new DateOnly(2026, 6, 10),
        ...
        var secondItem = PlanItem.Create(
            tripId,
            itemDate: new DateOnly(2026, 6, 11),
